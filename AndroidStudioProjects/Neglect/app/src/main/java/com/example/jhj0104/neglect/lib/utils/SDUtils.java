...
public class SDUtils {
    ...
    public static void handleOnResult(Context context, int requestCode, int resultCode, Intent data, SDUtils.handleOnResultDelegater delegater) {
    ...
    private static String getKey(int request_code) {
        return String.format(Locale.US, "SDUtils-%d", new Object[]{Integer.valueOf(request_code)});
    }
    ...
    public static int createStorageFileFD(Context context, Uri tree_uri, String mime, String file_name) {
        ...
        if(BuildCheck.isLollipop() && context != null && tree_uri != null && !TextUtils.isEmpty(file_name)) {
            ...
            try {
                ParcelFileDescriptor e = context.getContentResolver().openFileDescriptor(target.getUri(), "rw");
                return e != null?e.getFd():0;
